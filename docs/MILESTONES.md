...
> Last updated: 2026-07-14 (`northwind-sample-showcase` **LANDED**: D137–D140 — a three-page Northwind
> sample showcase, purely additive at the sample/example layer (no Core/EF/AspNetCore/adapter contract,
> route, envelope, or error change). The `a2n.Vista.Examples.AgGridNorthwind` host now serves all three pages
> behind a shared nav, reaching DynData "Table Browser" read-surface parity: **D137** — the single
> `AgGridNorthwind` host registers DataTables + QueryBuilder + AG Grid + OpenAPI, keeping
> `AllowAnonymousAccess()`; the standalone `Northwind` host stays the separate DataTables-only sample.
> **D138** — an additive read-only `GET /api/showcase/views` catalog (a pure `ShowcaseCatalog.Project` over
> `IViewRegistry`), secure-by-default. **D139** — static HTML + TypeScript (`tsc`, no bundler) with a
> `tsc --noEmit` gate + fast-check property tests for the pure transforms. **D140** — a third `vOrder` view
> so the set spans string/numeric/date/FK/composite-key. Pages: Simple Wiring (AG Grid `POST {route}/aggrid`),
> View Browser (DataTables + jQuery-QueryBuilder — dynamic columns from metadata, server-side paging + global
> search + multi-sort + advanced filter), and Custom Renderer (community `cellRenderer`s). Build green
> net8/9/10, **516 tests/TFM (net8) / 517 tests/TFM (net9/net10)** in `a2n.Vista.Tests` (+1, the
> `ShowcaseCatalog` CsCheck property) + **112 generator tests** unchanged (0 failed/skipped); the showcase
> host read (incl. a view-browser round-trip) + write + OpenAPI self-tests PASS. See §2 (Samples).
...
> → emit → write**): per-view `TRow`/`TCrud` DTOs, the fixed Vista envelopes, a presence-discriminated
> `FilterNode` union, the RFC 7807 `ProblemDetails` type, one re-lifted generic `ViewListResult<TRow>`/
> `PagedResult<TRow>` per view, and a per-view typed client over an injectable HTTP transport + auth
> provider. **D131** — the OpenAPI document is the single generation source (the generator references **no**
> Vista package). **D132** — secure-by-default posture: read facets default, write facets gated off by
> default; no embedded credential; HTTPS-default transport; every outcome a total, non-throwing discriminated
> `ClientResult<T>`. Two reconciliations against the live emitter (code is the oracle): `FilterNode` has no
> `discriminator` (presence-discriminated), and monomorphized `ViewListResult_*` envelopes are re-lifted to a
> generic. Additive-only (no server change). Build green net8/9/10, a **new `a2n.Vista.Client.TypeScript.Tests`
> = 136 tests/TFM** (0 failed/skipped) via CsCheck on the TUnit runner + a TypeScript generated-runtime
> property harness (fast-check under Node); the existing suites are unchanged (**448/450 tests/TFM** +
> **112 generator tests**), Northwind read + write + OpenAPI self-tests PASS unchanged. See §2 (M17).
> Prior: 2026-07-13 (`style-a-coverage` **LANDED**: the **final planned M9 Source Generator phase** —
...
Rough progress toward **v1.0 (production-ready): ~90%**. Foundation, the full server-half query engine,
...
| **Samples** | Northwind sample showcase — the `a2n.Vista.Examples.AgGridNorthwind` host became a **three-page showcase** behind a shared nav, at DynData "Table Browser" read-surface parity on the landed Vista contracts, purely additive at the sample/example layer (no Core/EF/AspNetCore/adapter contract, route, envelope, or error change). **D137:** the single `AgGridNorthwind` host serves all three pages and registers `DataTablesAdapter` + `QueryBuilderSchemaAdapter` + `AgGridAdapter` + the OpenAPI emitter, keeping `AllowAnonymousAccess()` (D94); the standalone `Northwind` host stays the separate DataTables-only single-view sample. **D138:** an additive read-only catalog endpoint `GET /api/showcase/views` — a pure `ShowcaseCatalog.Project(IViewRegistry)` → `ViewCatalogEntry(Name, Route, Title)` (`[]` on empty), secure-by-default, inside the host auth pipeline (no HTTP "list all views" existed before). **D139:** static HTML + TypeScript compiled by `tsc` (no bundler, emit to `wwwroot/js`), a `tsc --noEmit` gate, and fast-check property tests for the pure transforms (`columns.ts` metadata→columns, `search.ts` min-length gate). **D140:** a third read-only view `vOrder` so `vProductCategory`/`vOrderDetail`/`vOrder` span string/numeric/date/FK/composite-key. Pages: **Simple Wiring** (AG Grid infinite row model → `POST {route}/aggrid`, `?q=`), **View Browser** (DataTables.NET + jQuery-QueryBuilder — view selection, columns from `GET {route}/metadata`, server-side paging + min-length global search + single/multi sort + a `GET {route}/querybuilder`-driven advanced filter through `POST {route}/datatable`, dispose-on-switch, placeholder ⇒ no request, RFC 7807 ⇒ visible error + rows unchanged), and **Custom Renderer** (≥2 consumer-owned community `cellRenderer`s, presentation-only). The old standalone `AgGridSelfTest` was removed; the host self-test gained a view-browser round-trip (paging + global search + multi-sort + `jsonQB` in one request). | `northwind-sample-showcase` | D137, D138, D139, D140 |

**Verified at the Northwind sample showcase (D137–D140, 2026-07-14):** solution build green on net8/9/10,
**516 tests passing per TFM (net8) / 517 per TFM (net9/net10)** in `a2n.Vista.Tests` (+1 — the
`ShowcaseCatalog` CsCheck Property 2; the two fast-check properties `columns.ts`/`search.ts` run under Node)
+ **112** in `a2n.Vista.SourceGenerators.Tests` (0 failed, 0 skipped), and the showcase host read (incl. the
view-browser round-trip) + write + OpenAPI self-tests PASS (19 OpenAPI paths, 4 views). Additive-only — the
other example self-tests and every existing suite stay green and unchanged.
...
TUnit runner, plus a TypeScript generated-runtime property harness (fast-check under Node) against the
emitted client. The existing suites are unchanged — **448 tests/TFM (net8) / 450 (net9/net10)** in
`a2n.Vista.Tests` + **112** in `a2n.Vista.SourceGenerators.Tests` — and the Northwind read + write + OpenAPI
self-tests PASS unchanged (M17 is a pure downstream consumer; it touches no server code). The emitted OpenAPI
document is the authoritative oracle: the two headline parity harnesses (generated-type round-trip and schema
parity) plus the determinism/idempotence, type-mapping-fidelity, `$ref`-soundness, generic-re-lifting,
write-facet-gating, response-classification, authorization-enforcement, and no-embedded-credential properties
are the guard.
...
> serialization stays permanently RUC by design (D96/D130). And **M17** (the TypeScript client generator,
> D131/D132) has now **also landed** — a framework-agnostic typed client generated from the emitted OpenAPI
> document, a pure downstream consumer with no Vista project reference.
>
...
- Done: **M17** (TS client, D131/D132). Remaining: **M14–M15** (observability, versioning) and two major
  adapters from **M16** (AG Grid + MudBlazor). (**M11/M13** landed with D118; **M12** write path landed with
  D119/D120; **M18** OpenAPI landed with D127/D128.)
...
and the Style A coverage (M9-P6).
