...
2. **Do not enforce the whitelist.** The adapter **builds** the tree; the engine is what **rejects** (400) when a field/operator is not allowed. The adapter must not silently drop a leaf that "looks" invalid (except skipping non-field UI columns the *request itself declares* — see invariant 5) — so that the error contract stays consistent & the client gets feedback (in contrast to DynData, which silently skips, Spec 02 D60). **This governs the sort channel too**, not just the filter tree: an unresolvable sort key is passed through and rejected by the engine, never dropped (**D150**).
...
5. **Skip non-field columns — only where the transport declares them (D150).** UI columns are skipped when mapping columns→fields **only** when the request itself describes the column as one (e.g. DataTables `columns[i]` with `data=""`, or `orderable=false`/`searchable=false`, ref §7 item 6). A name the client asks the server to *act on* is never guessed at: DataTables carries a column table, so an action column is self-describing; an AG Grid `sortModel[].colId` / `filterModel` key declares nothing, so **every** entry is built and the engine decides. Matching a name against the projection inside the adapter *is* enforcing the whitelist (invariant 2), and it makes a caller's typo indistinguishable from a UI column — which returns `200` with an untouched row order instead of the `400` the same mistake earns on the filter channel.
