...
import { buildColumnDefs } from './columns.js';
...
function setGridPanelVisible(visible) {
...
async function fetchQueryBuilderFilters(route) {
...
function buildTableHeader(grid, columns) {
 ...
 */
async function initQueryBuilder(route) {
...
function readQbJson() {
    ...
    const rules = $(`#${ELEMENT_IDS.builder}`).queryBuilder('getRules', { skip_empty: true });
...
function firstSortableIndex(columns) {
 ...
 * rows unchanged (R3.9).
 */
function buildTable(entry, columns) {
    const grid = byId(ELEMENT_IDS.grid);
    if (!grid) {
    ...
    table.on('error.dt', (...args) => {
        const message = typeof args[3] === 'string' ? args[3] : 'unknown error';
        showStatus(`Request failed: ${message}. See the server console for the Problem Details response.`, true);
    });
    return table;
}
/**
 ...
 */
async function selectView(entry) {
    disposeCurrentView();
    clearStatus();
    setEmptyState(null);
...
function populateSelector(select, entries) {
    select.innerHTML = '';
...
function wireFilterButtons() {
    const applyBtn = byId(ELEMENT_IDS.applyFilter);
    ...
    const resetBtn = byId(ELEMENT_IDS.resetFilter);
    resetBtn?.addEventListener('click', () => {
        ...
        const searchInput = byId(ELEMENT_IDS.globalSearch);
        if (searchInput) {
            searchInput.value = '';
        }
...
function wireSelector(select, entries) {
 ...
 */
export async function initViewBrowser() {
    ...
    setGridPanelVisible(false);
    const select = byId(ELEMENT_IDS.viewSelector);
    wireGlobalSearch(byId(ELEMENT_IDS.globalSearch) ?? document.createElement('input'));
    wireFilterButtons();
...
//# sourceMappingURL=viewBrowser.js.map