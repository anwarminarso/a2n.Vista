...
/** Stable identifier for each showcase page, used to mark the active link. */
export type PageId = 'simple-wiring' | 'view-browser' | 'custom-renderer';

/** A single entry in the shared navigation bar. */
export interface NavItem {
  /** Stable page identifier (matches the `activePage` argument to {@link renderNav}). */
  readonly id: PageId;
  /** Human-readable link text shown in the nav bar. */
  readonly label: string;
  /** Relative URL of the page's HTML file, resolved against the current document. */
 ...
 */
export function renderNav(activePage: PageId, container: HTMLElement): void {
