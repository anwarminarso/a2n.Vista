...
public class NorthwindViews : ViewTemplate<NorthwindDbContext>
{
    ...
    protected override void Configure(IViewTemplateBuilder<NorthwindDbContext> views)
    {
        ...
        // Dated view (Decision Log D140): Orders exercises DateTime fields end to end so the QueryBuilder
        // date operators and date-aware column affordances are covered by the showcase. OrderId is the
        // single primary key and is kept visible as the natural display identifier (there is no business
        // "order name"). The foreign-key columns (CustomerId, EmployeeId, ShipVia) are hidden from
        // transport as done for the other views — they are technical join keys, not display data.
        views.AddView("vOrder", (db, sp) =>
                from o in db.Orders
                select new
                {
                    o.OrderId,
            ...
            .Field(x => x.OrderId, f => f.PrimaryKey())
            .Field(x => x.CustomerId, f => f.Hidden())
            .Field(x => x.EmployeeId, f => f.Hidden())
            .Field(x => x.ShipVia, f => f.Hidden());
        // No WithCrud(...) → read-only resource (List + Detail by OrderId).
    }
}
